...
BASE_DIR="./checkpoints/transformer/ablation/6_90"
MERTIC_DIR="./metrics/ablation2/6_90"
BATCH_PATH="./dining_dataset/batch_window18_stride9_v4"
...
find "$BASE_DIR" -type f -name "*.ckpt" | while read -r module_path; do
  ...
  python -m evaluation.evaluate --job=evaluate --module_path="$module_path" --test_idx="$test_idx" --metric_dir="$MERTIC_DIR" --batch_path="$BATCH_PATH"
